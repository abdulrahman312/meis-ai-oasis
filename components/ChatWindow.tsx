...
const ChatWindow: React.FC<ChatWindowProps> = ({ historyData, weatherData }) => {
  ...
  const toggleVoice = async () => {
    if (isLive) {
      ...
      [EXTERNAL RIYADH WEATHER] (Use ONLY if asked about Riyadh/City):
      - Temp: ${weatherData.temp}C
      - Condition: ${weatherData.condition}
      ` : "External weather unavailable.";

      let contextString = `${weatherContext}\nNo live sensor data is currently available.`;
      
      if (latest) {
        contextString = `
          ${weatherContext}

          [INTERNAL FARM SENSORS] (Priority for general queries):
  ...
  const handleSend = async () => {
    ...
    try {
      // PASS WEATHER DATA HERE
      const responseText = await sendMessageToGemini(userMsg.text, historyData, weatherData);
  ...
  return (
    <div className="flex flex-col h-full bg-white relative">
      ...
      <div className="p-3 border-b border-gray-100 bg-white flex items-center justify-between sticky top-0 z-10 shrink-0">
        ...
        {isLive && (
          <div className="flex items-center gap-2 px-2 py-0.5 bg-red-50 text-red-600 rounded-full border border-red-100 animate-pulse absolute right-2">
      ...
      <div 
        ...
        className="flex-1 overflow-y-auto p-3 space-y-4 custom-scrollbar bg-sadu-bg/50"
      >
        {messages.map((msg) => (
          <div 
            key={msg.id} 
          ...
          >
            ...
            >
              ...
              />
              <div className={`text-[10px] mt-1 text-right ${msg.role === 'user' ? 'text-gray-400' : 'text-gray-400'}`}>
           ...
           <div className="flex flex-col gap-2 opacity-80 pb-4">
             {liveUserText && (
               <div className="flex justify-end">
                  <div className="max-w-[90%] p-3 rounded-2xl text-sm leading-relaxed shadow-sm bg-sadu-dark/80 text-white rounded-br-none italic">
                    {liveUserText}...
                  </div>
               </div>
             )}
             {liveModelText && (
               <div className="flex justify-start">
                  <div className="max-w-[90%] p-3 rounded-2xl text-sm leading-relaxed shadow-sm bg-white/80 border border-gray-100 text-sadu-dark rounded-bl-none italic">
                     {liveModelText}...
                  </div>
               </div>
             )}
          ...
          <div className="flex justify-start">
            <div className="bg-white border border-gray-100 p-3 rounded-2xl rounded-bl-none shadow-sm">
              <div className="flex space-x-2">
                <div className="w-1.5 h-1.5 bg-sadu-oasis rounded-full animate-bounce"></div>
                <div className="w-1.5 h-1.5 bg-sadu-oasis rounded-full animate-bounce delay-75"></div>
                <div className="w-1.5 h-1.5 bg-sadu-oasis rounded-full animate-bounce delay-150"></div>
              </div>
            </div>
          </div>
        )}
      ...
      <div className="p-2 bg-white border-t border-gray-100 shrink-0">
        ...
        {isLive ? (
           <div className="flex items-center justify-between bg-red-50 border border-red-200 p-2 rounded-xl transition-all duration-300">
             <div className="flex items-center gap-3">
               ...
               <div className="flex flex-col">
                  <span className="text-sadu-dark font-bold text-xs">
                    {liveModelText ? "AI Speaking..." : (audioVolume > 10 ? "Listening..." : "Waiting for voice...")}
                  </span>
                  <span className="text-[10px] text-gray-500">Tap X to end session</span>
               </div>
             </div>
             
             <button 
               onClick={toggleVoice}
               className="p-1.5 bg-white text-sadu-dark rounded-full hover:bg-gray-100 shadow-sm border border-gray-200 transition-colors"
             >
               <X size={16} />
             </button>
           </div>
        ) : (
          <div className="flex gap-2 bg-gray-50 p-1 rounded-xl border border-gray-200 focus-within:border-sadu-oasis focus-within:ring-1 focus-within:ring-sadu-oasis/50 transition-all">
            <button
              onClick={toggleVoice}
              className="p-1.5 text-gray-500 hover:text-sadu-red hover:bg-red-50 rounded-lg transition-all"
            ...
            <input
              ...
              className="flex-1 bg-transparent px-2 py-1.5 text-sm text-sadu-dark focus:outline-none placeholder-gray-400"
            />
            
            <button
              onClick={handleSend}
              disabled={isLoading}
              className="bg-sadu-dark hover:bg-black text-white px-3 py-1.5 rounded-lg transition-all disabled:opacity-50 disabled:cursor-not-allowed flex items-center justify-center"
            >
              <Send size={16} />
            </button>
          </div>
